...
const toggleLayers = function(layer) {
    ...
    if (this.checked === true) layer.addTo(map);
...
countyPoly = new L.Shapefile("../data/AHVLACounties20120315.zip", {
    style: stylePoly("royalblue"),
...
riskAreaPoly = new L.Shapefile("../data/RiskAreas.zip", {
    style: styleRiskAreaPoly
});
...
legend.onAdd = function(map) {
    ...
    for(let i = 0; i < levels.length; i++)if (country[i] === "ENG") div.insertAdjacentHTML("beforeend", `<i style="background: ${colours[i]};"></i> ${levels[i]} <br>`);
    div.insertAdjacentHTML("beforeend", "<br>");
    // Wales
    div.insertAdjacentHTML("beforeend", "<strong>Wales</strong><br>");
    for(let i = 0; i < levels.length; i++)if (country[i] === "WAL") div.insertAdjacentHTML("beforeend", `<i style="background: ${colours[i]};"></i> ${levels[i]} <br>`);
    div.insertAdjacentHTML("beforeend", "<br>");
    // Scotland
    div.insertAdjacentHTML("beforeend", "<strong>Scotland</strong><br>");
    for(let i = 0; i < levels.length; i++)if (country[i] === "SCO") div.insertAdjacentHTML("beforeend", `<i style="background: ${colours[i]};"></i> ${levels[i]} <br>`);
    return div;
};
...
document.getElementById("btn_show-movements").addEventListener("click", function() {
    ...
    for(let i = 0; i < mov1.length; i++){
        cowMarker = new L.marker([
            mov1[i][0],
            mov1[i][1]
        ], {
            icon: cowIcon
        }); // leaflet marker object to store coordinates
    ...
    cattleMovLine = L.polyline(linePts, {
        color: "#0096FF"
    }).addTo(map); // create polyline object and plot on map
    ...
    once: true
}) // once: true means function is only executed once (on first click)
;
// ------------------------ //
//
...
//# sourceMappingURL=index.ff16d72e.js.map
