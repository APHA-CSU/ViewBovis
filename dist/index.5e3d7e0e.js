(function() {
    L.Control.LinearMeasurement = L.Control.extend({
        ...
        clickSpeed: 200,
        onAdd: function(map) {
            var container = L.DomUtil.create("div", "leaflet-control leaflet-bar"), link = L.DomUtil.create("a", "icon-ruler", container), map_container = map.getContainer(), me = this;
            link.href = "#";
            ...
            if (this.options.color && this.options.color.indexOf("#") === -1) this.options.color = "#" + this.options.color;
            else if (!this.options.color) this.options.color = "#4D90FE";
        ...
        initRuler: function() {
            ...
            this.moveEventFn = function(e) {
                if (!me.clickHandle) me.getMouseMoveHandler(e);
            };
        ...
        resetRuler: function(resetLayer) {
            ...
            if (resetLayer) {
                ...
                if (this.mainLayer) this._map.removeLayer(this.mainLayer);
                this.mainLayer = null;
            ...
            /* Leaflet return distances in meters */ this.UNIT_CONV = 1000;
        ...
        cleanUpMarkers: function(fixed) {
            ...
            if (layer) layer.eachLayer(function(l) {
                if (l.options && l.options.type === "tmp") {
                    if (fixed) l.options.type = "fixed";
                    else layer.removeLayer(l);
                }
            });
        },
        cleanUpFixed: function() {
            var layer = this.layer;
            if (layer) layer.eachLayer(function(l) {
                if (l.options && l.options.type === "fixed") layer.removeLayer(l);
            });
        },
        convertDots: function() {
            ...
            if (layer) layer.eachLayer(function(l) {
                if (l.options && l.options.type === "dot") {
                    var m = l.options.marker, i = m ? m.options.icon.options : null, il = i ? i.html : "";
                    if (il && il.indexOf(me.measure.unit) === -1) {
                        var str = l.options.label, s = str.split(" "), e = parseFloat(s[0]), u = s[1], label = "";
                        if (l.options.label.indexOf(me.measure.unit) !== -1) label = l.options.label;
                        else if (u === me.UNIT) label = (e * me.SUB_UNIT_CONV).toFixed(2) + " " + me.SUB_UNIT;
                        else if (u === me.SUB_UNIT) label = (e / me.SUB_UNIT_CONV).toFixed(2) + " " + me.UNIT;
                        var cicon = L.divIcon({
        ...
        displayMarkers: function(latlngs, multi, sum) {
            var x, y, label, ratio, p, latlng = latlngs[latlngs.length - 1], prevLatlng = latlngs[0], original = prevLatlng.distanceTo(latlng) / this.UNIT_CONV, dis = original;
            var p2 = this._map.latLngToContainerPoint(latlng), p1 = this._map.latLngToContainerPoint(prevLatlng), unit = 1;
            if (this.measure.unit === this.SUB_UNIT) {
                unit = this.SUB_UNIT_CONV;
                dis = dis * unit;
            }
            var t = sum * unit + dis, qu = sum * unit;
            for(var q = Math.floor(qu); q < t; q++){
                ratio = (t - q) / dis;
                if (q % this.separation || q < qu) continue;
                x = p2.x - ratio * (p2.x - p1.x);
                y = p2.y - ratio * (p2.y - p1.y);
                p = L.point(x, y);
                /* render a circle spaced by separation */ latlng = this._map.containerPointToLatLng(p);
        ...
        renderCircle: function(latLng, radius, layer, type, label) {
            var color = this.options.color, lineColor = this.options.color, azimut = "", nodeCls = "";
            type = type || "circle";
            linesHTML = [];
            var options = {
                color: lineColor,
            ...
            var a = this.prevLatlng ? this._map.latLngToContainerPoint(this.prevLatlng) : null, b = this._map.latLngToContainerPoint(latLng);
            if (type === "dot") {
                nodeCls = "node-label";
                if (a && this.options.show_azimut) azimut = ' <span class="azimut"> ' + this.lastAzimut + "&deg;</span>";
            }
        ...
        renderMultiPolyline: function(latLngs, dashArray, layer) {
            /* Leaflet version 1+ delegated the concept of multi-poly-line to the polyline */ var multi;
            if (L.version.startsWith("0")) multi = L.multiPolyline(latLngs, {
                color: this.options.color,
                weight: 2,
                opacity: 1,
                dashArray: dashArray
            });
            else multi = L.polyline(latLngs, {
                color: this.options.color,
                weight: 2,
                opacity: 1,
                dashArray: dashArray
        ...
        formatDistance: function(distance, precision) {
            var s = L.Util.formatNum(distance < 1 ? distance * parseFloat(this.SUB_UNIT_CONV) : distance, precision), u = distance < 1 ? this.SUB_UNIT : this.UNIT;
            return {
                scalar: s,
                unit: u
            };
        ...
        hasClass: function(target, classes) {
            ...
            for(var i in classes){
                if (fn(target, classes[i])) return true;
            }
            return false;
        },
        preClick: function(e) {
            var me = this, target = e.originalEvent.target;
        ...
        getMouseClickHandler: function(e) {
            ...
            if (me.poly) {
                ...
                if (!me.multi) me.multi = me.renderMultiPolyline(me.latlngsList, "5 5", me.layer, "dot");
                else me.multi.setLatLngs(me.latlngsList);
            }
            var o, dis;
            ...
            if (me.measure.unit === this.SUB_UNIT) dis = me.sum * me.SUB_UNIT_CONV;
            else dis = me.sum;
            var s = dis.toFixed(2);
        ...
        getMouseMoveHandler: function(e) {
            ...
            if (this.prevLatlng) {
                ...
                if (!this.poly) this.poly = this.renderPolyline(this.latlngs, "5 5", this.layer);
                else this.poly.setLatLngs(this.latlngs);
                /* Distance in miles/meters */ this.distance = parseFloat(this.prevLatlng.distanceTo(e.latlng)) / this.UNIT_CONV;
                /* scalar and unit */ this.measure = this.formatDistance(this.distance + this.sum, 2);
                var a = this.prevLatlng ? this._map.latLngToContainerPoint(this.prevLatlng) : null, b = this._map.latLngToContainerPoint(latLng);
                ...
                /* tooltip with total distance */ var label = this.measure.scalar + " " + this.measure.unit, html = '<span class="total-popup-content" style="background-color:' + this.options.color + "; color: " + this.options.contrastingColor + '">' + label + azimut + "</span>";
                if (!this.total) {
                ...
                /* Rules for separation using only distance criteria */ var ds = this.measure.scalar, old_separation = this.separation, digits = parseInt(ds).toString().length, num = Math.pow(10, digits), real = ds > num / 2 ? num / 10 : num / 20, dimension = 0;
                this.separation = real;
                /* If there is a change in the segment length we want to re-space
                   the dots on the multi line */ if (old_separation !== this.separation && this.fixedLast) {
                    this.cleanUpMarkers();
                    this.cleanUpFixed();
                    var multi_latlngs = this.multi.getLatLngs();
                    for(var s in multi_latlngs)dimension += this.displayMarkers.apply(this, [
                        multi_latlngs[s],
                        true,
                        dimension
                    ]);
                    ...
                    /* Review that the dots are in correct units */ this.convertDots();
                } else {
                    this.cleanUpMarkers();
                    this.displayMarkers.apply(this, [
                        this.poly.getLatLngs(),
        ...
        getDblClickHandler: function(e) {
            ...
            var workspace = this.layer, label = this.measure.scalar + " " + this.measure.unit + " ", total_scalar = this.measure.unit === this.SUB_UNIT ? this.measure.scalar / this.UNIT_CONV : this.measure.scalar, total_latlng = this.total.getLatLng(), total_label = this.total, html = [
        ...
        purgeLayers: function(layers) {
            for(var i in layers)if (layers[i]) this.layer.removeLayer(layers[i]);
        },
        layerSelected: function(e) {}
    });
})();

//# sourceMappingURL=index.5e3d7e0e.js.map
