...
L.Polyline.include({
    ...
    buildVectorHats: function(options) {
        ...
        if (this._arrowheads) this._arrowheads.remove();
        if (this._ghosts) this._ghosts.remove();
        //  -------------------------------------------------------- //
        //  ------------  FILTER THE OPTIONS ----------------------- //
        /*
        ...
        if (offsets?.start || offsets?.end) this._buildGhosts({
            start: offsets.start,
            end: offsets.end
        });
        ...
        lineToTrace._parts.forEach((peice, index)=>{
            ...
            const totalLength = (()=>{
                let total = 0;
                for(var i = 0; i < peice.length - 1; i++)total += this._map.distance(latlngs[i], latlngs[i + 1]);
                return total;
            })();
            ...
            } else if (isInPercent(frequency)) console.error("Error: arrowhead frequency option cannot be given in percent.  Try another unit.");
            ...
            const pushHats = (size, localHatOptions = {})=>{
                ...
                let hatPoints = [
                    ...
                    [
                        derivedLatLngs[i].lat,
                        derivedLatLngs[i].lng
                    ],
                    [
                        rightWingPoint.lat,
                        rightWingPoint.lng
            ...
            for(var i = 0; i < derivedLatLngs.length; i++){
                ...
                // ---- If size is chosen in percent ------------------------
                    ...
                    let hatSize = (()=>{
                        if (options.frequency === "endonly" && options.proportionalToTotal) return totalLength * sizePercent / 100;
                        else {
                ...
                } else if (isInPixels(size)) pushHatsFromPixels(options.size, perArrowheadOptions);
                else console.error("Error: Arrowhead size unit not defined.  Check your arrowhead options.");
    ...
    getArrowheads: function() {
        if (this._arrowheads) return this._arrowheads;
        else return console.error(`Error: You tried to call '.getArrowheads() on a shape that does not have a arrowhead.  Use '.arrowheads()' to add a arrowheads before trying to call '.getArrowheads()'`);
    },
    /**
	 ...
	 */ _buildGhosts: function({ start , end  }) {
        if (start || end) {
            ...
            latlngs = Array.isArray(latlngs[0]) ? latlngs : [
                latlngs
            ];
            const newLatLngs = latlngs.map((segment)=>{
                ...
                const totalLength = (()=>{
                    let total = 0;
                    for(var i = 0; i < segment.length - 1; i++)total += this._map.distance(segment[i], segment[i + 1]);
                    return total;
                })();
                // Modify latlngs to end at interpolated point
                if (start) {
                    let endOffsetInMeters = (()=>{
                        if (isInMeters(start)) return Number(start.slice(0, start.length - 1));
                ...
                if (end) {
                    let endOffsetInMeters = (()=>{
                        if (isInMeters(end)) return Number(end.slice(0, end.length - 1));
...
L.LayerGroup.include({
    removeLayer: function(layer) {
        ...
        if (this._map && this._layers[id]) {
            if (this._layers[id]._arrowheads) this._layers[id]._arrowheads.remove();
    ...
    onRemove: function(map, layer) {
        for(var layer in this._layers)if (this._layers[layer]) this._layers[layer].remove();
...
L.Map.include({
    removeLayer: function(layer) {
        ...
        if (layer._arrowheads) layer._arrowheads.remove();
        if (layer._ghosts) layer._ghosts.remove();
    ...
    geometryToLayer: function(geojson, options) {
        ...
        switch(geometry.type){
            ...
            case "MultiLineString":
                ...
                if (options.arrowheads) polyline.arrowheads(options.arrowheads);
                return polyline;
            case "Polygon":
            case "MultiPolygon":
...
//# sourceMappingURL=index.bdae0c02.js.map
